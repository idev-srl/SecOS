...
static int ramfs_add_common(const char* name, const void* data, size_t size, unsigned flags){
    ...
    e->flags = flags;
    if(size==0){ // evita kmalloc(0) per directory o file vuoto iniziale
        e->data = NULL;
        e->size = 0;
        return 0;
    }
    if(flags & 1){ // immutable: punta direttamente ai dati forniti
...
int ramfs_write(const char* name, size_t offset, const void* src, size_t len){
    ramfs_entry_t* e = (ramfs_entry_t*)ramfs_find(name); if(!e) return -1; if(e->flags & 1) return -1; // immutable
    if(offset > e->size) return -1; // no holes
...
int ramfs_remove(const char* name){
    for(size_t i=0;i<ramfs_count;i++){
        if(str_eq(ramfs_table[i].name,name)){
            if(ramfs_table[i].flags & 1) return -1; // immutable
            // free buffer
...
int ramfs_is_dir(const char* path){
    if(!path || path[0]==0 || (path[0]=='/' && path[1]==0)) return 1; // root treated as directory
...
size_t ramfs_list_path(const char* path, const ramfs_entry_t** out_array, size_t max){
    // Lista solo figli diretti di path (root: path=="" oppure "/" -> tutti top-level senza '/').
    ...
    for(size_t i=0;i<ramfs_count && count<max;i++){
        ...
        if(root){ // nessun '/'
            ...
            if(str_starts(name,path) && name[plen]=='/'){
                // figlio o discendente; accetta solo diretto (nessun altro '/').
...
static int path_validate_parent(const char* path){ // verifica genitore esiste se ha '/'
    for(size_t i=0; path[i]; i++){
        if(path[i]=='/'){
            ...
            const ramfs_entry_t* pe = ramfs_find(parent); if(!pe || !(pe->flags & 2)) return 0; // parent must be dir
        }
    }
    return 1;
}
int ramfs_mkdir(const char* path){
    if(!path || !*path) return -1; if(str_len(path)>=RAMFS_NAME_MAX) return -1; if(ramfs_find(path)) return -1; if(!path_validate_parent(path)) return -1; // parent check
    return ramfs_add_common(path,"",0,2); // directory flag
}
int ramfs_rmdir(const char* path){
    ramfs_entry_t* e = (ramfs_entry_t*)ramfs_find(path); if(!e) return -1; if(!(e->flags & 2)) return -1; if(e->flags & 1) return -1; // immutable dir not removable
    // check empty: no entries whose name starts with path + '/'
    size_t plen=str_len(path);
    for(size_t i=0;i<ramfs_count;i++){
...
int ramfs_rename(const char* old_path, const char* new_path){
    ...
    if(ramfs_find(new_path)) return -1; // nuovo esistente
    ramfs_entry_t* e = (ramfs_entry_t*)ramfs_find(old_path); if(!e) return -1; if(e->flags & 1) return -1; // immutabile no rename
    // Prevenzione cicli: rinominare una directory dentro se stessa (es: /a -> /a/b) non consentito.
    if((e->flags & 2) && str_starts(new_path, old_path)){
        size_t ol = str_len(old_path);
        if(new_path[ol]=='/' && new_path[ol+1]){
            // new_path ha old_path come prefisso seguito da '/' => destinazione discendente del sorgente
            return -1;
        }
    }
    // Validazione parent destinazione
    // Estrarre parent di new_path se ha '/'
    ...
    // Se directory con figli e new_path cambia prefisso, aggiornare figli
    ...
    if(is_dir){ size_t old_len = str_len(old_name); for(size_t i=0;i<ramfs_count;i++){ if(ramfs_table[i].name==e->name) continue; if(str_starts(ramfs_table[i].name, old_name) && ramfs_table[i].name[old_len]=='/'){
                // rebuild new child path: new_path + rest
...
int ramfs_init(void){ if(ramfs_inited) return 0; ramfs_inited=1; // file e dirs di esempio statici immutabili
    static const char readme_txt[] = "SecOS RAMFS\nQuesto e' un filesystem in memoria dimostrativo.\n";
    ...
    // directory di esempio (mutable)
    ramfs_mkdir("docs");
    ramfs_add("docs/info.txt","Documentazione RAMFS dir docs",29);
    // Metadata versione
    static char version_buf[128];
    // Popola versione dinamica (usa macro definite dal Makefile)
#ifdef BUILD_TS
    ...
    // Script di init: ogni riga comando shell
    ...
    // Manifest dinamico: elenco entries iniziali (dopo creazione base) sotto sys/manifest.txt
    {
        ...
        for(size_t i=0;i<n;i++){
            ...
            manifest_buf[mp++]='\n';
            if(mp>=sizeof(manifest_buf)-2) break; // evita overflow
        }
        manifest_buf[mp]=0;
