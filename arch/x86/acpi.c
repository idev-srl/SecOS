...
/* Walk the RSDT/XSDT pointer array and find the MADT ("APIC"). */
static uint64_t find_madt(uint64_t sdt, int is_xsdt){
    ...
    for(uint32_t i=0;i<n;i++){
        ...
        if(sig_eq(pv(tp),"APIC",4)) return tp;
    }
    return 0;
}
...
int acpi_init(uint64_t rsdp_hint){
    ...
    if(rev >= 2){                                          /* ACPI 2.0+: prefer XSDT */
        ...
        if(xsdt && sig_eq(pv(xsdt),"XSDT",4)) madt = find_madt(xsdt, 1);
    }
    if(!madt){                                             /* fall back to RSDT */
        uint64_t rsdt = rd32(r+16);
        if(rsdt && sig_eq(pv(rsdt),"RSDT",4)) madt = find_madt(rsdt, 0);
    }
