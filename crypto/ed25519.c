 ...
 *
 * Field arithmetic adapted from TweetNaCl (public domain).
 */
#include "ed25519.h"
#include "sha512.h"
...
static const gf
  ...
  Dc = {0x78a3,0x1359,0x4dca,0x75eb,0xd8ab,0x4141,0x0a4d,0x0070,0xe898,0x7779,0x4079,0x8cc7,0xfe73,0x2b6f,0x6cee,0x5203};

static const i64 L[32] = {0xed,0xd3,0xf5,0x5c,0x1a,0x63,0x12,0x58,0xd6,0x9c,0xf7,0xa2,0xde,0xf9,0xde,0x14,
...
static int unpackneg(gf r[4], const u8 p[32]) {
    ...
    set25519(r[2], gf1);
    unpack25519(r[1], p);
    S(num, r[1]); M(den, num, Dc); Z(num, num, r[2]); A(den, r[2], den);
...
static void modL(u8 *r, i64 x[64]) {
    ...
    carry = 0;
    FOR(j,32) { x[j] += carry - (x[31] >> 4) * L[j]; carry = x[j] >> 8; x[j] &= 255; }
...
int ed25519_verify(const uint8_t* msg, size_t mlen,
    ...
    if (unpackneg(q, pk)) return 0;           /* q = -A (invalid pubkey -> reject) */
    ...
    u8 t[32];
    pack(t, p);
    return crypto_verify_32(sig, t) == 0;      /* R == pack(p) ? */
}
