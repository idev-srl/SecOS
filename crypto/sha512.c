...
void sha512_init(sha512_ctx* c) {
    ...
    c->bitlen_lo = c->bitlen_hi = 0; c->buflen = 0;
}
...
void sha512_update(sha512_ctx* c, const void* data, size_t len) {
    ...
    while (len) {
        ...
        if (c->buflen == SHA512_BLOCK_SIZE) { sha512_block(c->state, c->buf); c->buflen = 0; }
    }
}
...
void sha512_final(sha512_ctx* c, uint8_t out[SHA512_DIGEST_SIZE]) {
    ...
    // 128-bit big-endian length, written directly into the buffer
    for (int i = 0; i < 8; i++) c->buf[112 + i] = (uint8_t)(hi >> (56 - i*8));
    for (int i = 0; i < 8; i++) c->buf[120 + i] = (uint8_t)(lo >> (56 - i*8));
    sha512_block(c->state, c->buf);
    for (int i = 0; i < 8; i++)
        for (int j = 0; j < 8; j++)
