...
**Current real state (HEAD):** M0–M7 done. M7 (ring-3 entry + `SYS_YIELD` cooperative scheduling) **works**: two ring-3 processes alternate via `SYS_YIELD`. The demo is gated behind `M7_RING3_DEMO` (off by default, so normal boot reaches the shell). Build the demo with `make CFLAGS_EXTRA=-DM7_RING3_DEMO=1`. Verify everything non-interactively with `tools/selftest.sh` (asserts M4 12/12 isolation selftest + alternating ring-3 switches + no CPU exception, from the debugcon log alone). CPU exceptions are surfaced on debugcon as `[EXC] int=… rip=…` for headless diagnosis. The detailed execution plan is `docs/DEVELOPMENT_PLAN.md`; next is M8 (preemptive multitasking + `SYS_EXIT` + serial). See `docs/devlog/M7.md` for the four bugs M7 uncovered.
