...
- Demos are off by default so normal boot reaches the shell (the signing gate is enforced but dormant at boot). Verify non-interactively with `tools/selftest.sh` (M4 + crypto KATs + M7 + M8 + M9 + M10×{fat32,ext2,ext4} + M11 + M12 + M13 + M14 = 61/61, from the debugcon log). CPU exceptions surface on debugcon as `[EXC] int=… rip=…`. Interactive shell over serial: `make run-serial`; with a disk: `make disk-fat32 && make run-disk` (note: `-boot d` so the BIOS boots the CD not the data disk). Execution plan: `docs/DEVELOPMENT_PLAN.md`. M0–M14 done (phases A–E of the plan complete; M14 is stretch); remaining work is stretch/hardening (copy-on-write/page-out, drop low identity map, real `DRIVER_OP_MAP_MEM`/IRQ-to-driver, ext4 journaling).
