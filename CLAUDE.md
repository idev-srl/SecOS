...
- **M8** (preemptive multitasking) core works: trapframe `isr_timer` + quantum preemption (ring-3 only), kernel idle task, `SYS_EXIT`→zombie→reap, `vmm_space_destroy` frees the private PDPT, no PMM leak. Demo gated behind `M8_SCHED_DEMO`. **Known issue:** N≥3 spawn corruption (a process's `PML4[0]` is zeroed when the next is created after a few rounds — a frame-aliasing/identity-map bug, not the scheduler); demo uses N=2; this is the first M9 item. See `docs/devlog/M8.md`.
