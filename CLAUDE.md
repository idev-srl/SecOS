...
- Demos are off by default so normal boot reaches the shell (the signing gate is enforced but dormant at boot). Verify non-interactively with `tools/selftest.sh` (M4 + crypto KATs + M7 + M8 + M9 + M10×{fat32,ext2,ext4} + M11 = 47/47, from the debugcon log). CPU exceptions surface on debugcon as `[EXC] int=… rip=…`. Interactive shell over serial: `make run-serial`; with a disk: `make disk-fat32 && make run-disk` (note: `-boot d` so the BIOS boots the CD not the data disk). Execution plan: `docs/DEVELOPMENT_PLAN.md`; next is M12 (higher-half + buddy/multi-frame allocator + demand paging).
