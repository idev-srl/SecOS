...
- Demos are off by default so normal boot reaches the shell (the signing gate is enforced but dormant at boot). Verify non-interactively with `tools/selftest.sh` (M4 + crypto KATs + M7 + M8 + M9 signed-userland = 14/14, from the debugcon log). CPU exceptions surface on debugcon as `[EXC] int=… rip=…`. Interactive shell over serial: `make run-serial`. Execution plan: `docs/DEVELOPMENT_PLAN.md`; next is M10 (storage & persistence, virtio-blk).
