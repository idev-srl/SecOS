#!/bin/bash
# Compila bootloader UEFI minimale usando gnu-efi
# Basato su toolchain che compila systemd-boot

set -e

cd "$(dirname "$0")"

BOOTLOADER_C="uefi/boot_minimal.c"
OUTPUT="dist/EFI/BOOT/BOOTX64.EFI"

# Crea versione minimale del bootloader che fa boot del kernel
cat > "$BOOTLOADER_C" << 'UEFI_CODE'
...
# Compila con gnu-efi
echo "Building with gnu-efi..."
gcc -ffreestanding -fshort-wchar -O2 -mno-red-zone -m64 -fno-stack-protector \
    -I/usr/include/efi -I/usr/include/efi/x86_64 \
    -fPIC -fno-strict-aliasing -fcf-protection=none \
    -c "$BOOTLOADER_C" -o /tmp/boot_minimal.o

ld -nostdlib -znocombreloc -T /usr/lib/elf_x86_64_efi.lds \
    -shared -Bsymbolic \
    -L/usr/lib \
    /usr/lib/crt0-efi-x86_64.o /tmp/boot_minimal.o \
    -o /tmp/boot_minimal.elf \
    -lgnuefi -lefi

objcopy -j .text -j .sdata -j .data -j .dynamic -j .dynsym \
    -j .rel -j .rela -j .reloc \
    --target=efi-app-x86_64 \
    /tmp/boot_minimal.elf "$OUTPUT"

echo "Bootloader built: $OUTPUT"
file "$OUTPUT"
